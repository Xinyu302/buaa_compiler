...
void genCalMips(CalMidCode* calMidCode) {
    ...
    if (nowFuncSymbolTable->isConstValue(calMidCode->left, value1)) {
        ...
        if (regLeft.length()) {
            ...
            regLeft = sRegPool->getReg(calMidCode->left);
            if (regLeft.length()) {
                l = regLeft;
            } else {
    ...
    if (nowFuncSymbolTable->isConstValue(calMidCode->right,value2)) {
        ...
        regRight = tRegPool->checkRegByName(calMidCode->right);
        if (regRight.length()) {
            r = regRight;
        }
        else {
            regRight = sRegPool->getReg(calMidCode->right);
            if (regRight.length()) {
                r = regRight;
            } else {
        ...
        if (isValue1const && anOperator == MidCode::PLUS) {
            genThreeRegInstr("addiu",regResult,r,int2string(value1));
            goto genCalMipsEnd;
        }
    }
...
void genScanMips(ReadMidCode* readMidCode)
{
    ...
    const std::string &reg = sRegPool->getReg(readMidCode->result);
    if (reg.length()) {
        genMove(reg, "$v0");
    } else {
    ...
    if ((writeMidCode->num).length()) {
        ...
        if (nowFuncSymbolTable->isConstValue(writeMidCode->num,value)) {
            ...
            if (reg.length()) {
                ...
                reg = sRegPool->getReg(writeMidCode->num) ;
                if (reg.length()) {
                    genMove("$a0", reg);
                } else {
...
void genAssignMips(AssignMidCode* assignMidCode) {
    std::string reg = "";
    std::string result = assignMidCode->result;
    if (j + 1 < size && midCodeVec[j+1]->getMidCodeClass()==MidCode::ASSIGNMIDCODE) {
        AssignMidCode *ass = (AssignMidCode *) midCodeVec[j+1];
        if (ass->leftVal == assignMidCode->result) {
            sRegPool->getReg(assignMidCode->result);
            tRegPool->checkRegByName(assignMidCode->result);
            result = ass->result;
            j = j + 1;
//            goto assend;
        }
    ...
    if (assignMidCode->getAssignType() == AssignMidCode::IMMEDIATE) {
        if (nowFuncSymbolTable != globalSymbolTable) {
            reg = sRegPool->getReg(result);
        }
        if (reg.length()) {
            genLi(reg, assignMidCode->leftImmediate);
        } else {
    ...
    if (assignMidCode->getAssignType() == AssignMidCode::VAL) {
        if (assignMidCode->leftVal == "$v0") { // drectly load $v0 to temp var result
            if (nowFuncSymbolTable->isTmpValue(result) && tRegPool->hasFreeReg()) {
                const std::string &regResult = tRegPool->allocReg(result);
                if (regResult.length()) {
            ...
            if (reg.length()) {
                ...
                genMoveVarToMem(result,"$v0");
            }
            return;
        }
        else {
            reg = tRegPool->checkRegByName(assignMidCode->leftVal);
            std::string reg1 = sRegPool->getReg(result);
            if (reg.length()) {
                if (reg1.length()) {
                    genMove(reg1, reg);
                } else {
                    genMoveVarToMem(result, reg);
                }
            }
            else {
                reg = sRegPool->getReg(assignMidCode->leftVal);
                if (reg.length()) {
                    if (reg1.length()) {
                        genMove(reg1, reg);
                    } else {
...
void genCompareMips(CompareMidCode* compareMidCode) {
    ...
    if (nowFuncSymbolTable->isConstValue(compareMidCode->left,value)) {
        ...
        if (reg.length()) {
            ...
            std::string reg1 = sRegPool->getReg(compareMidCode->left);
            if (reg1.length()) {
                l = reg1;
            } else {
    ...
    if (nowFuncSymbolTable->isConstValue(compareMidCode->right,value)) {
        ...
        if (reg.length()) {
            ...
            std::string reg2 = sRegPool->getReg(compareMidCode->right);
            if (reg2.length()) {
                r = reg2;
            } else {
...
void genPushMips(PushMidCode* pushMidCode) {
    int value;
    bool regA = false;
    std::string rega = "$a" + int2string(pushMidCode->index + 1);
    std::string reg2push = "$t0";
    if (pushMidCode->index < 3) {
        regA = true;
    }
    ...
    if (regA) {
        if (reg2push[1] == 'a') {
            genLw(rega, "$sp",
                  calledFuncSymbolTable->getOffset((reg2push)) - calledFuncSymbolTable->getSubOffset());
        } else {
            genMove("$a" + int2string(pushMidCode->index + 1), reg2push);
        }
    } else {
        const std::string &paraname = calledFuncSymbolTable->getParaName(pushMidCode->index);
        std::string regPara = calledFuncSymbolTable->getSRegPool()->getReg(paraname);
        if (regPara.length()) {
            if (reg2push[1] == 'a') {
                genLw(regPara, "$sp",
                      calledFuncSymbolTable->getOffset((reg2push)) - calledFuncSymbolTable->getSubOffset());
            } else {
                genMove(regPara, reg2push);
            }
//            genMove(regPara, reg2push);
        } else {
            genSw(reg2push, "$sp",
...
void genAssignRegFromExp(const std::string& reg,const std::string& exp) {
    ...
    if (nowFuncSymbolTable->isConstValue(exp,value)) {
        ...
        if (reg1.length()) {
            ...
            std::string reg2 = sRegPool->getReg(exp);
            if (reg2.length()) {
                genMove(reg, reg2);
            } else {
